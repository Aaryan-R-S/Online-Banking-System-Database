...
create table minor_account(
	...
	account_number bigint unsigned not null, -- minor's account# (put pan_number='XXXXXXXXXX') - Leave it
...
alter table card auto_increment=100000000000; -- 12 digit card number
...
-- table for closed accounts
create table closed_account(
	account_number bigint unsigned not null,
	date_of_closing date not null default (curdate()),
    
    primary key (account_number),
    foreign key (account_number) references bank_account(account_number)
);

-- table for blocked cards
